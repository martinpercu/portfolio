...
export class ProjectsService {
  ...
  constructor() {
    ...
    this.projects_ai = [
      {
        title: 'TalentGraph',
        description: 'AI-powered recruitment platform built with Angular 19 and a LangGraph agent backend. Features intelligent candidate screening with streaming AI conversations, automated resume parsing and scoring, voice input/output (STT/TTS), MCP integrations (Google Calendar + Gmail), and a comprehensive recruiter dashboard for full talent pipeline management.',
        problem: 'The core intelligence is a production-ready LangGraph StateGraph with 34 specialized nodes organized in 9 categories and 15 conditional routes. It uses a multi-model strategy: GPT-4o-mini for conversations and Claude Haiku for fast domain classification (~50-100ms). A multi-level detection system (triggers → MCP → silent load → pending action → job mention → candidate → job list) enables fast paths under 10ms — some with NO LLM at all. PostgreSQL checkpointing via LangGraph PostgresSaver persists full conversation state across sessions. Voice features include VAD via Web Audio API, Groq Whisper for STT, and Kokoro ONNX for TTS with gapless playback and auto-listen mode.',
        year: 2025,
        image: 'talent-graph-light',
        techs: [this.techs[0], this.techs[5], this.techs[6], this.techs[43], this.techs[16], this.techs[44], this.techs[41], this.techs[13], this.techs[11], this.techs[47], this.techs[32], this.techs[34]],
        techs2nd: [this.techs[2], this.techs[3]],
        frontend: true,
        backend: true,
        githubOk: true,
        githubUrl: 'https://github.com/martinpercu/talent-graph',
        webOk: true,
        webUrl: 'https://talent-graph.web.app',
        github_text: 'Front code here.',
        github: 'https://github.com/martinpercu/talent-graph',
        github2nd_text: 'Back full Architecture.',
        github2nd: 'https://github.com/martinpercu/Talent-Graph/blob/main/docs/AGENT_ARCHITECTURE.md',
        // webdev_text: 'Recruiter dashboard (login required).',
        // webdev: 'https://talent-graph.web.app/recruiter',
        web_text: 'Live Demo Platform.',
        web: 'https://talent-graph.web.app',
      },
      {
        ...
        webdev_text: 'The provisional Dash for Admin.',
        webdev: 'https://trainer-teacher.web.app/main',
        web_text: 'Pick your teacher... Work Ethics, Washington or Vertical Seal.',
